...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # most likely position for each ghost
        ghosts = []
        for ghostPosition in livingGhostPositionDistributions:
            ghosts.append(ghostPosition.argMax())

        # want the ghost with the minimum distance
        nearest = float("inf")
        returnGhost = None
        for ghost in ghosts:
        ...
        # want action that goes toward the closest ghost
        actionValue = float("inf")
        returnAction = None
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            value = self.distancer.getDistance(successor, returnGhost)
